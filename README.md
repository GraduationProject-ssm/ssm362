...
随着计算机技术的发展以及计算机网络的逐渐普及，互联网成为人们查找信息的重要场所，二十一世纪是信息的时代，所以信息的管理显得特别重要。因此，使用计算机来管理设备配件管理和设备检修系统的相关信息成为必然。开发合适的设备配件管理和设备检修系统，可以方便管理人员对设备配件管理和设备检修系统的管理，提高信息管理工作效率及查询效率，有利于更好的为人们服务。

1.2研究目的

随着互联网技术的快速发展，网络时代的到来，网络信息也将会改变当今社会。各行各业在日常企业经营管理等方面也在慢慢的向规范化和网络化趋势汇合。设备配件管理和设备检修系统的信息化程度体现在将互联网与信息技术应用于经营与管理，以现代化工具代替传统手工作业。无疑，使用网络信息化管理使信息管理更先进、更高效、更科学，信息交流更迅速。

对于之前设备配件管理和设备检修系统的管理，大部分都是使用传统的人工方式去管理，这样导致了管理效率低下、出错频率高。而且，时间一长的话，积累下来的数据信息不容易保存，对于查询、更新还有维护会带来不少问题。对于数据交接也存在很大的隐患。如果采用电子化的存储方式就会带来很大的改善，而且给用户的查询带来了很大便利，因此设计一个设备配件管理和设备检修系统刻不容缓，能够提高信息的管理水平。
## 1.3系统设计思想
...
# 3系统分析
## 3.1可行性分析
通过对本设备配件管理和设备检修系统实行的目的初步调查和分析，提出可行性方案并对其一一进行论证。我们在这里主要从技术可行性、经济可行性、操作可行性等方面进行分析。
### 3.1.1技术可行性
本设备配件管理和设备检修系统采用Spring Boot框架，JAVA作为开发语言，是基于WEB平台的B/S架构系统。
...
因此，设备配件管理和设备检修系统在开发技术上具有很高可行性，且开发人员掌握了一定的开发技术，所以此系统的开发技术具有可行性。
### 3.1.2经济可行性
本设备配件管理和设备检修系统采用的软件都是开源的，这样能够削减很多的精力和资源，降低开发成本。同时对计算机的配置要求也极低，即使是淘汰下来的计算机也能够满足需要，因此，本系统在经济上是完全具有可行性的，所以在经济上是十分可行的。
### 3.1.3操作可行性
本设备配件管理和设备检修系统的界面简单易操作，用户只要平时有在用过电脑，都能进行访问和操作。本系统具有易操作、易管理、交互性好的特点，在操作上是非常简单的，因此在操作上具有很高的可行性。

综上所述，此系统开发目标已明确，在技术、经济和操作方面都具有很高的可行性，并且投入少、功能完善、管理方便，因此系统的开发是完全可行的。
## 3.2系统性能分析
### 3.2.1 系统安全性
此设备配件管理和设备检修系统要严格控制管理权限，具体要求如下：

（1）要想对设备配件管理和设备检修系统进行管理，首先要依靠用户名和密码在系统中登陆，无权限的用户不可以通过任何方式登录系统和对系统的任何信息和数据进行查看，这样可以保证系统的安全可靠性和准确性。
...
图4.14 管理员信息实体属性图
### 4.3.2数据库设计表
设备配件管理和设备检修系统需要后台数据库，下面介绍数据库中的各个表的详细信息：
...
|jianxiuqingkuang|longtext|是|NULL|检修情况|
|jianxiugongjieguo|varchar(200)|是|NULL|检修工结果|
|jianxiuriqi|date|是|NULL|检修日期|
...
|shebeibianhao|varchar(200)|是|NULL|设备编号|
|shebeimingcheng|varchar(200)|是|NULL|设备名称|
|jianxiutixing|longtext|是|NULL|检修提醒|
|tixingshijian|datetime|是|NULL|提醒时间|
|zhanghao|varchar(200)|是|NULL|账号|
|xingming|varchar(200)|是|NULL|姓名|
表4.3 设备信息
...
|guigexinghao|varchar(200)|否||规格型号|
|anquankucun|int(11)|否||安全库存|
|jianxiucishu|int(11)|是|NULL|检修次数|
|shoumingshengyu|varchar(200)|是|NULL|寿命剩余|
表4.4 使用计划
...
|shebeibianhao|varchar(200)|是|NULL|设备编号|
|shebeimingcheng|varchar(200)|是|NULL|设备名称|
|kaishishiyongshijian|date|是|NULL|开始使用时间|
|jihuashiyongshijian|date|是|NULL|计划使用时间|
...
# 5系统详细实现
## 5.1 维修员管理
设备配件管理和设备检修系统的系统管理员可以对维修员添加修改删除以及查询操作。具体界面的展示如图5.1所示。

![](/md/blog.008.png)

图5.1 维修员管理界面
## 5.2 设备信息管理
管理员可以对设备信息进行添加修改删除操作。具体界面如图5.2所示。

![](/md/blog.009.png)

图5.2 设备信息界面
## 5.3 使用计划管理
管理员可以对使用计划信息修改删除操作。界面如下图所示：

![](/md/blog.010.png)

图5.3 使用计划信息界面
## 5.4 检修提醒管理
维修员可以在检修提醒里面查看检修提醒消息。界面如下图所示：
